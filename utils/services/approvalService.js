...
const handleUserApproval = async (request, updateUser, tx) => {
  ...
  // Helper function to normalize autoGenerated pay components
  const normalizeAutoGenerated = async (autoGenerated) => {
    if (!autoGenerated?.payComponents?.length) return autoGenerated;
  ...
  switch (action) {
    case "create":
      ...
      if (roles.length > 0) {
        ...
        let normalizedAutoGeneratedData = null;

        // Normalize autoGenerated data if employee role exists
        if (roleNames.includes(ROLES_LIST.EMPLOYEE) && autoGenerated) {
          console.log("[create] Normalizing autoGenerated data for employee");
          normalizedAutoGeneratedData = await normalizeAutoGenerated(
            autoGenerated
          );
          console.log(
            "[create] Normalized autoGenerated:",
            normalizedAutoGeneratedData
          );
        }
        ...
        for (const roleName of roleNames) {
          ...
          switch (roleName) {
            case ROLES_LIST.ADMIN:
              ...
              break;
            case ROLES_LIST.CUSTOMER:
              await tx.customer.create({ data: { userId: user.id } });
              break;
            case ROLES_LIST.EMPLOYEE:
              // Include normalized autoGenerated data for employee role
              await tx.employee.create({
                data: {
                  userId: user.id,
                  autoGenerated: normalizedAutoGeneratedData || null,
                },
              });
              console.log(
                "[create] Employee record created with autoGenerated data"
              );
              break;
            case ROLES_LIST.CONTRACTOR:
    ...
    case "edit":
      ...
      if (editRoles.length > 0) {
        ...
        // Normalize autoGenerated data if employee role exists
        if (
          roleNames.includes(ROLES_LIST.EMPLOYEE) &&
          editAutoGenerated !== undefined
        ) {
          console.log("[edit] Handling autoGenerated data for employee");
          if (editAutoGenerated === null) {
            console.log("[edit] Clearing autoGenerated data");
            normalizedAutoGeneratedData = null;
          } else {
            console.log("[edit] Normalizing autoGenerated data");
            normalizedAutoGeneratedData = await normalizeAutoGenerated(
              editAutoGenerated
            );
            console.log(
              "[edit] Normalized autoGenerated:",
              normalizedAutoGeneratedData
            );
          }
        }
        ...
        for (const roleName of roleNames) {
          ...
          switch (roleName) {
            ...
            case ROLES_LIST.EMPLOYEE:
              ...
              console.log(
                "[edit] Employee record updated with autoGenerated data"
              );
              break;
            case ROLES_LIST.CONTRACTOR:
              // Include commission data for contractor role
      ...
      } else if (editAutoGenerated !== undefined) {
        // Handle autoGenerated update even if roles didn't change
        console.log("[edit] Only updating autoGenerated data");
        ...
        if (editAutoGenerated === null) {
          ...
          const normalizedAutoGeneratedData = await normalizeAutoGenerated(
            editAutoGenerated
          );
          if (existingEmployee) {
            await tx.employee.update({
              where: { userId: user.id },
              data: { autoGenerated: normalizedAutoGeneratedData },
            });
          } else {
            await tx.employee.create({
              data: {
                userId: user.id,
                autoGenerated: normalizedAutoGeneratedData,
              },
            });
          }
        }
...
const handleJobOrderApproval = async (request, updateUser, tx) => {
  ...
  switch (action) {
    ...
    case "edit":
      ...
      if (!payload) {
        console.error("[edit] Payload is undefined");
        throw new Error("Invalid payload for edit operation");
      }

      // Debug the payload structure
      console.log("[edit] Full payload:", JSON.stringify(payload));

      // Check if payload is a string (it might be stored as JSON string)
      let parsedPayload = payload;
      if (typeof payload === "string") {
        try {
          parsedPayload = JSON.parse(payload);
          console.log("[edit] Successfully parsed payload from string");
        } catch (parseError) {
          console.error("[edit] Failed to parse payload as JSON:", parseError);
          throw new Error("Invalid payload format for edit operation");
        }
      }

      // Now destructure from parsedPayload
      const {
        updateData = {},
      ...
      // Fix the condition - check if updateData exists and has contractorId
      if (
        updateData &&
        ...
        labor:
          updateData?.labor !== undefined
            ? updateData.labor &&
              ...
              : null
            : existingJobOrder.labor,
        contractorPercent: editContractorPercent,
        updatedByUser: updateUser,
      };

      // Handle status update if provided
      if (updateData?.status !== undefined) {
        editData.status = updateData.status;
      }
...
const approveRequest = async (requestId, updateUser) => {
  const request = await prisma.approvalLog.findUnique({
    where: { id: requestId },
  });
    ...
    default:
      ...
      switch (request.actionType) {
        ...
        case "edit":
          ...
          if (!existingRecord) {
            console.log(
              `[approveRequest] Record with ID ${request.recordId} not found in ${tableName} for edit - skipping`
            );
            // Don't throw error, just skip since record might have been deleted
            break;
          }
        ...
        case "delete":
          // First check if record exists before deleting
          const recordToDelete = await prisma[tableName].findUnique({
            where: { id: request.recordId },
          });
          if (!recordToDelete) {
            console.log(
              `[approveRequest] Record with ID ${request.recordId} not found in ${tableName} for delete - already deleted`
            );
            // Don't throw error, just skip since record is already deleted
            break;
          }
          ...
          console.log(
            `[approveRequest] Successfully deleted record ${request.recordId} from ${tableName}`
          );
          break;
      }
  }

  // Update the approval log status regardless of whether the action was performed
  return prisma.approvalLog.update({
    where: { id: requestId },
    data: {
      status: "published",
      approvedByUser: updateUser,
      responseComment:
        request.actionType === "delete"
          ? `Record ${request.recordId} was already deleted or not found - request marked as complete.`
