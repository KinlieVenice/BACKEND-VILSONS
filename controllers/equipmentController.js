...
const createEquipment = async (req, res) => {
    ...
    if (!name || !quantity || !price || !branchId) return res.status(404).json({ message: "Name, quantity, price, and branchId are required"});
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await tx.equipmentEdit.create({
                data: {...equipmentData, requestType: "create", equipmentId: null}
            }) : await tx.equipment.create({
                 data: equipmentData
            })

            message = needsApproval ? "Equipment awaiting approval" : "Equipment successfully added";
            return equipment
        })
...
const editEquipment = async (req, res) => {
    ...
    try {
        const equipment = await prisma.equipment.findFirst({ where: { id: req.body.id }});
        if (!equipment) return res.status(404).json({ message: `Equipment with ${req.body.id} not found`});
        ...
        const result = await prisma.$transaction(async (tx) => {
            const editedEquipment = needsApproval
              ...
              : await tx.equipment.update({
                ...
                message = needsApproval ? "Equipment edit is awaiting approval" : "Equipment successfully edited"
...
module.exports = { createEquipment, editEquipment, deleteEquipment }