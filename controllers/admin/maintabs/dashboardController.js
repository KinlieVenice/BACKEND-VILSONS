...
const getCustomerBalanceNEWW = async (req, res) => {
  try {
    let { month, year, branch } = req.query;
...
const getRecentJobOrdersOld = async (req, res) => {
  try {
...
const getRevenueAndProfitChartOLd = async (req, res) => {
  try {
...
const getRevenueAndProfitChartNEW = async (req, res) => {
  try {
    // Default: current year and monthly view
    const now = new Date();
    let { year, type } = req.query;
...
const getRecentJobOrders = async (req, res) => {
  ...
  try {
    const baseWhere = { ...branchFilter("jobOrder", branch, req.branchIds) };
    ...
    const statuses = ["pending", "ongoing", "completed", "forRelease", "archived"];
