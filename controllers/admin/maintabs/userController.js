...
const createUserNew = async (req, res) => {
  ...
  try {
    ...
    const isContractor = roleBaseNames.some(
      (roleName) => roleName === ROLES_LIST.CONTRACTOR
    );
    const isEmployee = roleBaseNames.some(
      (roleName) => roleName === ROLES_LIST.EMPLOYEE
    );
    const hasCustomerRole = roleBaseNames.some(
      (roleName) => roleName === ROLES_LIST.CUSTOMER
    );
...
const createUserNOAUTOPAY = async (req, res) => {
  const parsedBody = parseArrayFields(req.body, ["roles", "branches"]);
  const {
    name,
    username,
...
const createUser = async (req, res) => {
  ...
  try {
    ...
    // Validate autoGenerated if employee role is selected
    if (isEmployee && autoGenerated) {
      // If autoGenerated is provided, validate it
      if (typeof autoGenerated !== "object" || autoGenerated === null) {
        return res.status(400).json({
          message: "AutoPayment must be an object",
        });
      }

      if (!autoGenerated.branchId) {
        return res.status(400).json({
          message: "branchId is required in autoGenerated",
        });
      }
      ...
      if (autoGenerated.branchId) {
        ...
        if (!branchExists) return res.status(400).json({
          message: "branchId doesnt exist",
        });
      }

      if (
        !autoGenerated.payComponents ||
        !Array.isArray(autoGenerated.payComponents)
      ) {
        return res.status(400).json({
          message: "payComponents must be an array in autoGenerated",
        });
      }

      // Validate each autoGenerated item
       for (const component of autoGenerated.payComponents) {
         if (!component.componentId || !component.amount) {
           return res.status(400).json({
             message: "Each pay component must have componentId and amount",
           });
         }

         if (isNaN(Number(component.amount)) || Number(component.amount) < 0) {
           return res
             .status(400)
         ...
         if (component.componentId) {
           const componentExists = await prisma.component.findFirst({
             where: { id: component.componentId },
           });

           if (!componentExists)
             return res.status(400).json({
               message: `Pay Component with ID ${component.componentId} does not exist`,
             });
         }

         if (component.schedule) {
           if (!Array.isArray(component.schedule)) {
             return res.status(400).json({
           ...
           const invalidDays = component.schedule.filter((day) => {
             const num = Number(day);
             return isNaN(num) || !Number.isInteger(num) || num < 1 || num > 31;
           });

           if (invalidDays.length > 0) {
             return res.status(400).json({
               message: `Invalid days in schedule: ${invalidDays.join(
                 ", "
               )}. Must be numbers 1-31`,
             });
           }

           // Remove duplicates and sort
           component.schedule = [...new Set(component.schedule.map(Number))].sort(
             (a, b) => a - b
           );
         }
       }
    ...
    // Include commission and autoGenerated in userData for approval
    const userData = {
      fullName: name,
      username,
      phone: phone.toString(),
      ...
      ...(needsApproval
        ? {
            ...
            ...(isEmployee && autoGenerated && { autoGenerated }), // Include autoGenerated for employees
          }
        : {}),
      image,
    };
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!needsApproval) {
        const roleTableMap = {
          ...
          customer: () => tx.customer.create({ data: { userId: user.id } }),
          employee: () => {
            // Create employee with autoGenerated data if provided
            return tx.employee.create({
              data: {
                userId: user.id,
                ...(autoGenerated && { autoGenerated }), // Include autoGenerated for employees
              },
            });
          },
          contractor: () => {
    ...
    return res.status(201).json({
      ...
      data: needsApproval
        ? {
            ...
            fullName: result.user.payload.fullName,
            ...(isContractor && { commission: Number(commission) }),
            ...(isEmployee && autoGenerated && { autoGenerated }), // Include autoGenerated in response
          }
        : {
            userId: result.userDetails.id,
            username: result.userDetails.username,
